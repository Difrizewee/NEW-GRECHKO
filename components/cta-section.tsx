...
import { ClipboardList, PhoneCall, Send } from "lucide-react";
...
export default function CtaSection() {
  ...
  const requestSummary = items.map((i) => `${i.name} ×${i.qty}`).join(", ");
        ...
        <div className="grid grid-cols-1 lg:grid-cols-5 gap-6">
                ...
                {items.length > 0 && (
                  <div className="rounded-lg bg-primary/5 border border-primary/15 p-4 flex items-start gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-muted-foreground">{requestSummary}</p>
          ...
          <div className="lg:col-span-2 flex flex-col gap-5">
            <div className="glass-panel rounded-2xl p-6 md:p-8 flex flex-col items-center text-center flex-1 justify-center">
              ...
              <p className="text-xs text-muted-foreground mt-3">{SITE.workHours}</p>
            </div>
